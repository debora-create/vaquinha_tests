...
namespace Vaquinha.Tests.Common.Fixtures
{
    ...
    public class DoacaoFixture
    {
        ...
        public Doacao DoacaoValida(bool emailInvalido = false, double? valor = 5, bool maxLenghField = false, bool aceitaTaxa = false, bool querParcelar = false)
        {            
            ...
            faker.CustomInstantiator(f => new Doacao(Guid.Empty, Guid.Empty, Guid.Empty, valor ?? (double)f.Finance.Amount(MIN_VALUE, MAX_VALUE, DECIMALS), 
                                                        PessoaValida(emailInvalido, maxLenghField), null, null, aceitaTaxa, querParcelar));
